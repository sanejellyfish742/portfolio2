...
const updatePage = async () => {
  ...
  const filteredArray = fruitsArray.filter(checkSugar);
  
  function checkSugar(sugar) {
  return sugar.nutritions.sugar > 15;
}
...
const newElement = document.createElement('div');
  newElement.innerHTML = filteredArray.map(object => object.name);
...
const exampleAddElement = () => {
  ...
  // Append the new element to an existing part of the webpage
  const existingElement = document.getElementById('exampl');
  existingElement.append(newElement);
}